...
이번 프로젝트는 여기서 완결. 목표(gold standard 도구로 클러스터링+세포타입
주석까지)는 다 달성했고, 조건 비교/sub-clustering 등은 다음 프로젝트로 넘김.

bulk 때와 비교하면:
- bulk: 논문 재현이 목표 -> 정확한 수치(정렬률, 유의 유전자 수) 비교가 핵심이었음
- single-cell: 표준 도구 검증이 목표 -> "알려진 생물학적 패턴이 재현되는가"가 핵심
- 둘 다 gold standard 도구 선택 -> 실행 -> 검증 -> 문서화 흐름은 동일하게 적용됨
